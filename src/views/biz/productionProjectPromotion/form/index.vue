...
<template>
  ...
  >
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="生产立项编号" prop="projectnum" v-on:click.native="chooseProductionProject">
          <el-input v-model="form.variables.projectnum" disabled placeholder="请选择生产立项" style="width:100%;">
            <el-button
              @click="chooseProductionProject"
              slot="append"
      ...
      <el-col :span="12">
        <el-form-item label="项目名称" prop="projectname">
          <el-input v-model="form.variables.projectname" disabled placeholder="系统自动带入" style="width:100%;" />
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="项目类型" prop="leixing">
          <el-input v-model="form.variables.leixing" disabled placeholder="系统自动带入" style="width:100%;" />
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item label="建设（投资）方" prop="jianshefang">
          <el-input v-model="form.variables.jianshefang" disabled placeholder="系统自动带入" style="width:100%;" />
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="推进时间" prop="yanqingshijian">
          <el-date-picker
            v-model="form.variables.yanqingshijian"
            type="date"
            placeholder="选择推进时间"
            align="right"
      ...
      <el-col :span="12">
        <el-form-item label="推进用途" prop="yongtu">
          <el-select
          ...
          >
            <el-option
              v-for="item in formPromotionPurposeList"
              :key="item.dictValue"
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="24">
        <el-form-item label="备注说明" prop="yanqingshuoming">
          <el-input
            v-model="form.variables.yanqingshuoming"
            type="textarea"
            :rows="2"
            :disabled="!isformdisabled"
            placeholder="请输入备注说明"
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      ...
      <el-col :span="12">
        <el-form-item label="本次推进成本(元)" prop="yusuan">
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="24">
        <el-form-item label="推进统计" class="title_class">
          <el-divider></el-divider>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="推进预算上限(元)" prop="zongyusuan">
          <el-input v-model="form.variables.zongyusuan" disabled placeholder="系统自动带入" style="width:100%;"/>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item label="已累计报销金额(元)" prop="zbaoxiaojine">
          <el-input v-model="form.variables.zbaoxiaojine" disabled placeholder="系统自动带入" style="width:100%;"/>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="剩余推进额度(元)" prop="bencigongguanjine">
          <el-input v-model="form.variables.bencigongguanjine" disabled placeholder="系统自动计算" style="width:100%;"/>
        </el-form-item>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="4" style="padding:0 10px;" v-if="isChoose">
      <el-col :span="24">
        <ProductionProjectPopup @choosedProect="getProductionProject" req-url="/system/productionproject/myAndTrackMan" />
      </el-col>
    </el-row>
...
<script>
import { getHisCostByProjectNo, save } from '@/api/biz/productionProjectPromotion'
...
export default {
  components: { ProductionProjectPopup, candidateUsersDialog },
  props: {
    proData: {
      type: String,
      default: () => {
  ...
  data() {
    return {
      ...
      fileData: {
        fieldName: 'fujian',
        tableName: 'pms_production_project_promotion',
      },
      formUserName: '',
      formDeptName: '',
      form: {
      ...
      rules: {
        projectnum: [
          { required: true, message: '请选择生产立项编号', trigger: 'blur' },
        ],
        renshu: [
          { required: true, message: '请输入推进人员', trigger: 'blur' },
        ],
        jiushuitype: [
        ...
        yanqingshijian: [
          { required: true, message: '请选择推进时间', trigger: 'blur' },
        ],
  ...
  created() {
    // 推进用途
    this.getDicts('promotion_purpose').then((res) => {
      this.formPromotionPurposeList = res.data
    })
    // 材料种类
  ...
  methods: {
    // 字典翻译
    dictFormat(dictList,code) {
      return this.selectDictLabel(dictList, code)
    },
    gettreeselect() {
      treeselect().then((res) => {
    ...
    fillCostByProjectNo(projectNo,total){
      getHisCostByProjectNo(projectNo).then((res) => {
        ...
        this.totalPromotion = res.data.totalPromotion
        // this.form.variables.bencigongguanjine = parseFloat(total - reimbursement - hisCost).toFixed(2);
      })
    },
    ...
    getProductionProject(row) {
      if (row) {
        ...
        this.form.variables.leixing = this.dictFormat(this.formProjectTypeList,row.projectType1)
        this.form.variables.jianshefang = row.construction
        this.form.variables.zongyusuan = row.guanlifei
        this.form.variables.bencigongguanjine = row.remainderMoney ? row.remainderMoney : row.guanlifei
        this.fillCostByProjectNo(row.projectCode,row.guanlifei)
      }
      this.isChoose = false
    },
    downloadFile({ url }) {
 ...
 this.form.variables.fujian += `;${data[0].fileId}`
    ...
    onSubmit() {
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          if(parseFloat(this.form.variables.yusuan) > parseFloat(this.form.variables.bencigongguanjine)){
            this.msgError('本次公关金额已超出剩余公关预算')
          }else if(parseFloat(this.totalPromotion) + parseFloat(this.form.variables.yusuan) > parseFloat(this.form.variables.zongyusuan)){
            this.msgError('仍有推进处于审批中，累计公关金额已超出公关预算上限')
          }else{
            ...
            if (procInsId != '' && typeof (procInsId) != 'undefined' && procInsId != 'undefined') {
              ...
              let totalCost = this.form.variables.yusuan
              let peopleNum = this.form.variables.renshu
              let per = totalCost/peopleNum
              let selected = '0'
              if(totalCost > 2000 || per > 200){
                selected = '1'
              }
              this.completeData.selected = selected
  ...
  computed: {
    computeTotalCost: function () {
      let materialCost =
        this.form.variables.jiushuitype == null
          ? 0
          : this.form.variables.jiushuitype
      let materialNum =
      ...
      this.form.variables.yusuan = totalCost
      return totalCost
    },
