          ...
          <div v-show="ifCardShow">
            ...
            <div class="my-col">
              <el-form-item label="建设（投资）方" prop="jianshefang">
                <el-input
                  v-model="queryParams.jianshefang"
              ...
              <el-form-item label="推进成本(元)" prop="yusuan">
                <el-input
                  v-model="queryParams.yusuan"
                  placeholder="请输入推进成本"
                  clearable
                  size="small"
                  @keyup.enter.native="handleQuery"
...
<script>
import { list ,delProductionprojectpromotion,exportProjectPromotion } from '@/api/biz/productionProjectPromotion'
...
export default {
  ...
  data() {
    return {
      formPath: '/task/record/index/productionProjectPromotion',
      flowId: 'process_42xjseup',
      ifCardShow: false,
      showTime: '',
      flowtitle: '',
      flowopen: false,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm(
        ...
        return delProductionprojectpromotion(ids)
      }).then(() => {
        this.getList()
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出推进申请数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return exportProjectPromotion(queryParams)
      }).then(response => {
