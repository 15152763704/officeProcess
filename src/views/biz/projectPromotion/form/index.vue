...
<template>
  ...
  >
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="跟踪立项编号" prop="yingxiaono" >
          <el-input v-model="form.variables.yingxiaono" disabled placeholder="请选择跟踪立项" style="width:100%;">
            <el-button
              v-on:click.native="isformdisabled && chooseTrackPorject()"
              slot="append"
      ...
      <el-col :span="12">
        <el-form-item label="项目名称" prop="xiangmuname">
          <el-input v-model="form.variables.xiangmuname" disabled placeholder="系统自动带入" style="width:100%;"/>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="项目类型" prop="xiangmuleixing">
          <el-select
            v-model="form.variables.xiangmuleixing"
            disabled
            clearable
            placeholder="项目类型"
            style="width:100%;"
      ...
      <el-col :span="12">
        <el-form-item label="建设（投资）方" prop="touzifang">
          <el-input v-model="form.variables.touzifang" disabled placeholder="系统自动带入" style="width:100%;"/>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="投资规模" prop="touziguimo">
          <el-input-number v-model="form.variables.touziguimo" :precision="2" disabled placeholder="系统自动带入"
                           style="width:100%;"
          />
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item label="投资性质" prop="touzixingzhi">
          <el-input v-model="form.variables.touzixingzhi" disabled placeholder="系统自动带入" style="width:100%;"/>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="4" style="padding:0 10px;">
    ...
    <el-row :gutter="4" style="padding:0 10px;">
      <el-col :span="12">
        <el-form-item label="剩余推进额度(元)" prop="bencijine">
          <el-input v-model="form.variables.bencijine" disabled placeholder="系统自动计算" style="width:100%;"/>
        </el-form-item>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="4" style="padding:0 10px;" v-if="isChoose">
      <el-col :span="24">
        <TrackProjectPopup @transferTrackProjectData="getTrackProject" req-url="/system/trackproject/listForCreator" />
      </el-col>
    </el-row>
...
<script>
import { getHisCostByTrackNo, save } from '@/api/biz/projectPromotion'
...
export default {
  components: { TrackProjectPopup, candidateUsersDialog },
  props: {
    proData: {
      type: String,
      default: () => {
  ...
  data() {
    return {
      ...
      fileData: {
        fieldName: 'fujian',
        tableName: 'pms_project_promotion'
      },
      formUserName: '',
      formDeptName: '',
      form: {
      ...
      rules: {
        yingxiaono: [
          { required: true, message: '请选择跟踪立项编号', trigger: 'blur' }
        ],
        gongguanrenshu: [
  ...
  methods: {
    ...
    fillCostByTrackNo(trackNo, total) {
      getHisCostByTrackNo(trackNo).then((res) => {
        ...
        // this.form.variables.bencijine = parseFloat(total - reimbursement - hisCost).toFixed(2);
      })
    },
    ...
    getTrackProject(row) {
      if (row) {
        ...
        this.form.variables.xiangmuname = row.projectName
        // this.form.variables.xiangmuleixing = this.dictFormat(this.formProjectTypeList, row.leixing)为什么要转成明文，路庆武20220102修改成id
        this.form.variables.xiangmuleixing = row.leixing
        this.form.variables.touzifang = row.jianshedanwei
        this.form.variables.touziguimo = row.guimo
        this.form.variables.touzixingzhi = this.dictFormat(this.formInvestmentList, row.touzixingzhi)
        this.form.variables.zongyusuan = row.xtfsx
        this.form.variables.bencijine = row.remainderMoney ? row.remainderMoney : row.xtfsx
        this.fillCostByTrackNo(row.numbers, row.xtfsx)
      }
      this.isChoose = false
    },
    downloadFile({ url }) {
 ...
 this.form.variables.fujian += `;${data[0].fileId}`
    ...
    onSubmit() {
      this.$refs['form'].validate(async(valid) => {
        if (valid) {
          if (parseFloat(this.form.variables.jine) > parseFloat(this.form.variables.bencijine)) {
            this.msgError('本次公关金额已超出剩余公关预算')
          }else if(parseFloat(this.totalPromotion) + parseFloat(this.form.variables.jine) > parseFloat(this.form.variables.zongyusuan)){
