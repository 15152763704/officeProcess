<template>
  <div class="app-container">
      ...
      <el-card :body-style="{ padding: '14px 15px 7px' }" class="box-card">
        ...
        <el-collapse-transition>
          <div v-show="ifCardShow">
            <div class="my-col">
              <el-form-item label="报销编号" prop="xmbxnums">
                <el-input
                  v-model="queryParams.xmbxnums"
                  clearable
                  placeholder="请输入报销编号"
                  size="small"
                  @keyup.enter.native="handleQuery"
    ...
    >
      ...
      <el-table-column align="center" label="报销编号" prop="xmbxnums" sortable/>
      <el-table-column align="center" label="项目名称" prop="projectname" sortable/>
      <el-table-column align="center" label="申请事由" prop="xmbxshiyou" sortable/>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listProjectbaoxiao(this.queryParams).then((response) => {
    ...
    handleUpdate(row) {
      ...
      getProjectbaoxiao(id).then((response) => {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateProjectbaoxiao(this.form).then((response) => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addProjectbaoxiao(this.form).then((response) => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有费用报销数据项?', '警告', {
        ...
        .then(function() {
          return exportProjectbaoxiao(queryParams)
        })
        .then((response) => {
