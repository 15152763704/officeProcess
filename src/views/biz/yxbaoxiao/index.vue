<template>
  <div class="app-container">
          ...
          <div v-show="ifCardShow">
            <div class="my-col">
              <el-form-item label="营销报销编号" prop="yxnums">
                <el-input
                  v-model="queryParams.yxnums"
                  clearable
                  placeholder="请输入营销报销编号"
                  size="small"
                  @keyup.enter.native="handleQuery"
                />
              </el-form-item>
              ...
              <el-form-item label="报支单位" prop="baoxiaodanwei">
                <el-select
                  v-model="queryParams.baoxiaodanwei"
                  clearable
                  placeholder="请选择报支单位"
                  size="small"
                >
            ...
            <div class="my-col">
              ...
              <el-form-item label="报销部门" prop="createDeptName">
                <el-input
    ...
    >
      ...
      <el-table-column align="center" label="推进/投标报销编号" prop="yxnums" sortable/>
      <el-table-column align="center" label="报销事由" prop="baoxiaoshiyou" sortable/>
      <el-table-column :formatter="danweiFormat" align="center" label="报支单位" prop="baoxiaodanwei" sortable/>
      ...
      <el-table-column align="center" label="报销部门" prop="createDeptName" sortable/>
      <el-table-column
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        >
          ...
          <el-table-column label="发票金额(元)" prop="billjine">
            <template slot-scope="scope">
              <el-input v-model="scope.row.billjine" placeholder="请输入发票金额(元)"/>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listYxbaoxiao(this.queryParams).then((response) => {
    ...
    danweiFormat(row, column) {
      return this.selectDictLabel(this.danweiOptions, row.baoxiaodanwei)
    },
    // 流程状态字典翻译
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateYxbaoxiao(this.form).then((response) => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addYxbaoxiao(this.form).then((response) => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有费用报销数据项?', '警告', {
        ...
        .then(function() {
          return exportYxbaoxiao(queryParams)
        })
        .then((response) => {
