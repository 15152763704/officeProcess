<template>
  <div class="app-container">
    ...
    >
      <el-card :body-style="{ padding: '14px 15px 7px' }" class="box-card">
        <div slot="header" class="clearfix">
          ...
          <el-form-item label="借款总金额(元)" prop="zmoney">
            <el-input
              v-model="queryParams.zmoney"
              clearable
    ...
    <el-table v-loading="loading" :data="travelreimbursementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="借款总金额(元)" prop="zmoney" sortable/>
      <el-table-column align="center" label="本次还款金额(元)" prop="hmoney" sortable/>
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="借款总金额(元)" prop="zmoney">
          <el-input v-model="form.zmoney" placeholder="请输入借款总金额(元)"/>
        </el-form-item>
        <el-form-item label="本次还款金额(元)" prop="hmoney">
        ...
        <el-form-item label="待还款金额" prop="daihuankuan">
          <el-input v-model="form.daihuankuan" placeholder="请输入待还款金额"/>
        </el-form-item>
...
import {
  ...
  delTravelreimbursement,
  exportTravelreimbursement,
  getTravelreimbursement,
  listTravelreimbursement,
  updateTravelreimbursement
} from '@/api/biz/travelreimbursement'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listTravelreimbursement(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getTravelreimbursement(id).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTravelreimbursement(this.form).then(response => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addTravelreimbursement(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除还款申请编号为"' + ids + '"的数据项?', '警告', {
        ...
        return delTravelreimbursement(ids)
      }).then(() => {
        this.getList()
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有还款申请数据项?', '警告', {
        ...
        return exportTravelreimbursement(queryParams)
      }).then(response => {
