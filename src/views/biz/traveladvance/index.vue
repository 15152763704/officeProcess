<template>
  <div class="app-container">
        ...
        <el-collapse-transition>
          <div v-show="ifCardShow">
            <div class="my-col">
              ...
              <el-form-item label="借款事由" prop="yuanyin">
                <el-input
              ...
              <el-form-item label="借款单位" prop="jkdanwei">
                <el-select v-model="queryParams.jkdanwei" clearable placeholder="请选择借款单位" size="small" style="width: 100%;">
                  <el-option
                    v-for="dict in jkdanweiOptions"
                    :key="dict.dictValue"
            ...
            <div class="my-col">
              ...
              <el-form-item label="是否还清" prop="sfhw">
                <el-select v-model="queryParams.sfhw" clearable placeholder="请选择是否还清" size="small" style="width: 100%;">
                  <el-option
    ...
    <el-table v-loading="loading" :data="traveladvanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="借款事由" prop="yuanyin" sortable/>
      <el-table-column :formatter="jkdanweiFormat" align="center" label="借款单位" prop="jkdanwei" sortable/>
      <el-table-column align="center" label="借款金额(元)" prop="money" sortable/>
      <el-table-column :formatter="sfhwFormat" align="center" label="是否还清" prop="sfhw" sortable/>
      ...
      <el-table-column align="center" label="申请时间" prop="createTime" sortable width="180">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="借款事由" prop="yuanyin">
          <el-input v-model="form.yuanyin" placeholder="请输入内容" type="textarea"/>
        </el-form-item>
        ...
        <el-form-item label="借款单位" prop="jkdanwei">
          <el-select v-model="form.jkdanwei" placeholder="请选择借款单位">
            <el-option
              v-for="dict in jkdanweiOptions"
              :key="dict.dictValue"
              :label="dict.dictLabel"
...
export default {
  ...
  created() {
    ...
    this.getDicts('form_company_name').then(response => {
      this.jkdanweiOptions = response.data
    })
  ...
  methods: {
    ...
    getList() {
      ...
      listTraveladvance(this.queryParams).then(response => {
    ...
    sfhwFormat(row, column) {
      return this.selectDictLabel(this.sfhwOptions, row.sfhw)
    },
    // 用途类型字典翻译
    ...
    // 借款单位字典翻译
    jkdanweiFormat(row, column) {
      return this.selectDictLabel(this.jkdanweiOptions, row.jkdanwei)
    },
    // 流程状态字典翻译
    ...
    handleUpdate(row) {
      ...
      getTraveladvance(id).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTraveladvance(this.form).then(response => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addTraveladvance(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有borrow数据项?', '警告', {
        ...
        return exportTraveladvance(queryParams)
      }).then(response => {
