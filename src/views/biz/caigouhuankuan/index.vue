<template>
  <div class="app-container">
      ...
      <el-card :body-style="{ padding: '14px 15px 7px' }" class="box-card">
        <div slot="header" class="clearfix">
          ...
          <el-form-item label="借款事由" prop="jiekuanshiyou">
            <el-input
              v-model="queryParams.jiekuanshiyou"
              clearable
              placeholder="请输入借款事由"
              size="small"
              @keyup.enter.native="handleQuery"
          ...
          <div v-show="ifCardShow">
            <div class="my-col">
              <el-form-item label="借款单位" prop="jiekuandanwei">
                <el-select
                  v-model="queryParams.jiekuandanwei"
                  clearable
                  placeholder="请选择借款单位"
                  style="width:100%;"
                >
                  <el-option
                    v-for="item in hkdanweiOptions"
                    :key="item.dictValue"
              ...
              <el-form-item label="借款金额(元)" prop="jiekuanjinex">
                <el-input
                  v-model="queryParams.jiekuanjinex"
                  clearable
              ...
              <el-form-item label="本次还款金额(元)" prop="huankuanjinex">
                <el-input
                  v-model="queryParams.huankuanjinex"
                  clearable
              ...
              <el-form-item label="剩余还款金额(元)" prop="weihuankuanjinex">
                <el-input
                  v-model="queryParams.weihuankuanjinex"
                  clearable
    ...
    <el-table v-loading="loading" :data="travelreimbursementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="借款人" prop="createUserName" sortable/>
      <el-table-column align="center" label="部门" prop="createDeptName" sortable/>
      <el-table-column align="center" label="借款事由" prop="jiekuanshiyou" sortable/>
      <el-table-column :formatter="hkdanweiFormat" align="center" label="借款单位" prop="jiekuandanwei" sortable/>
      <el-table-column align="center" label="借款金额(元)" prop="jiekuanjinex" sortable/>
      <el-table-column align="center" label="本次还款金额(元)" prop="huankuanjinex" sortable/>
      <el-table-column align="center" label="剩余还款金额(元)" prop="weihuankuanjinex" sortable/>
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="借款金额大写" prop="jkjinedx">
          <el-input v-model="form.jkjinedx" placeholder="请输入借款金额(元)大写"/>
        </el-form-item>
        ...
        <el-form-item label="还款金额大写" prop="hkjinedx">
          <el-input v-model="form.hkjinedx" placeholder="请输入还款金额大写"/>
        </el-form-item>
        <el-form-item label="未还款金额大写" prop="whkjinedx">
          <el-input v-model="form.whkjinedx" placeholder="请输入未还款金额大写"/>
        </el-form-item>
...
import {
  addCaigouhuankuan,
  delCaigouhuankuan,
  exportCaigouhuankuan,
  getCaigouhuankuan,
  listCaigouhuankuan,
  updateCaigouhuankuan
} from '@/api/biz/caigouhuankuan'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listCaigouhuankuan(this.queryParams).then(response => {
    ...
    hkdanweiFormat(row, column) {
      return this.selectDictLabel(this.hkdanweiOptions, row.jiekuandanwei)
    },
    // 付款方式字典翻译
    ...
    handleUpdate(row) {
      ...
      getCaigouhuankuan(id).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCaigouhuankuan(this.form).then(response => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addCaigouhuankuan(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有还款申请数据项?', '警告', {
        ...
        return exportCaigouhuankuan(queryParams)
      }).then(response => {
