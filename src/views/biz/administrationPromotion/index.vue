...
<script>
import { list,delAdministrationpromotion } from '@/api/biz/administrationPromotion'
...
export default {
  ...
  data() {
    return {
      formPath: '/task/record/index/administrationPromotion',
      flowId: 'process_yxczbdld',
      ifCardShow: false,
      showTime: '',
      flowtitle: '',
      flowopen: false,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm(
        ...
        return delAdministrationpromotion(ids)
      }).then(() => {
        this.getList()
