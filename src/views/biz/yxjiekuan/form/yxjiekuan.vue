  ...
  <div>
    ...
    >
      ...
      <el-row :gutter="4" style="padding:0 10px;">
        <el-col :span="12">
          <el-form-item label="推进/投标借款编号" prop="nums">
            <el-input
      ...
      <el-row :gutter="4" style="padding:0 10px;">
        ...
        <el-col :span="12">
          <template v-if="isShowNumDiv">
            <el-form-item v-if="form.variables.leixing==1" :label="'跟踪立项编号'" prop="toubiaonum">
              <el-input
            ...
            <el-form-item v-if="form.variables.leixing!=1" :label="'投标立项编号'" prop="toubiaonum">
              <el-input
                v-model="form.variables.toubiaonum"
      ...
      <el-row :gutter="4" style="padding:0 10px;">
        ...
        <el-col :span="12">
          <el-form-item label="收款单位" prop="receiveMoneyCompany">
            <el-input
              v-model="form.variables.receiveMoneyCompany"
              :disabled="!isformdisabled"
              style="width:100%;"
            />
          </el-form-item>
    ...
    <TrackProjectPopup v-if="isChoose === 'TrackProjectPopup'"
...
import { addYxjiekuan, updateYxjiekuan } from '@/api/biz/yxjiekuan'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        receiveMoneyCompany:[{ required: true, message: '请输入收款单位', trigger: 'blur' }],
        beizhu: [
          {
            required: true,
            message: '请输入备注',
  ...
  methods: {
    clearNum() {
      this.form.variables.toubiaonum = null
      ...
      if(row){
        const keymap = new Map([
          [
            '1',
            () => ({
              toubiaonum: row.numbers,
              toubiaoproject: row.projectName
            })
          ],
          [
            '2',
        ...
        const result =
          row &&
          keymap.get(this.form.variables.leixing) &&
          keymap.get(this.form.variables.leixing)()
        this.form.variables = {
          ...this.form.variables,
          ...result
        }
        this.form.variables.xtfsx = row.xtfsx
        this.fillCostByTrackNo(this.form.variables.toubiaonum,row.xtfsx)
      }
      this.isChoose = ''
    },
      ...
      this.$refs['form'].validate(async(valid) => {
        if (valid) {
          ...
          if (procInsId != '' && typeof (procInsId) != 'undefined' && procInsId != 'undefined') {
            ...
            const updateSealmsg = await updateYxjiekuan({
              id: this.form.id,
              ...this.form.variables
            })
            ...
            if(leixing == '1' && parseFloat(jiekuanjine) > parseFloat(promotionLeft)){
              ...
              const { code, data } = await addYxjiekuan(this.form.variables)
              if (code == 200) {
                this.form.id = data
