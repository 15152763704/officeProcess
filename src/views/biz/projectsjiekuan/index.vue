<template>
  <div class="app-container">
      ...
      <el-card :body-style="{ padding: '14px 15px 7px' }" class="box-card">
        ...
        <el-collapse-transition>
          <div v-show="ifCardShow">
            ...
            <div class="my-col">
              <el-form-item label="是否还完" prop="sfhq">
                <el-select
    ...
    <el-table v-loading="loading" :data="traveladvanceList" border @selection-change="handleSelectionChange">
...
import {
  addProjectsjiekuan,
  delProjectsjiekuan,
  exportProjectsjiekuan,
  getProjectsjiekuan,
  listProjectsjiekuan,
  updateProjectsjiekuan
} from '@/api/biz/projectsjiekuan'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // }
      listProjectsjiekuan(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getProjectsjiekuan(id).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateProjectsjiekuan(this.form).then(response => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addProjectsjiekuan(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有borrow数据项?', '警告', {
        ...
        return exportProjectsjiekuan(queryParams)
      }).then(response => {
