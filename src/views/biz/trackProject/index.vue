<template>
  <div class="app-container">
      ...
      <el-card class="box-card" :body-style="{ padding: '14px 15px 7px' }">
        ...
        <el-collapse-transition>
          <div v-show="ifCardShow">
            <div class="my-col">
              <el-form-item label="项目编号" prop="numbers">
                <el-input
            ...
            <div class="my-col">
              ...
              <el-form-item label="投资性质" prop="touzixingzhi">
                <el-select
                  v-model="queryParams.touzixingzhi"
                  clearable
                  placeholder="请输入投资性质"
                  @keyup.enter.native="handleQuery"
                >
                  <el-option
                    v-for="item in formTouzixingzhiList"
                    :key="item.dictValue"
                    :label="item.dictLabel"
                    :value="item.dictValue"
                  ></el-option>
              ...
              <el-form-item label="投资规模(万元)" prop="guimo">
                <el-input
                  v-model="queryParams.guimo"
                  placeholder="请输入投资规模"
                  clearable
                  size="small"
                  @keyup.enter.native="handleQuery"
    ...
    <el-table v-loading="loading" :data="modelList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="项目编号" sortable align="center" prop="numbers"/>
      <el-table-column label="项目名称" sortable align="center" prop="projectName"/>
      <el-table-column label="项目类型" sortable align="center" prop="leixing" :formatter="leixingFormat"/>
      <el-table-column label="建设（投资）方" sortable align="center" prop="jianshedanwei"/>
      <el-table-column label="投资性质" sortable align="center" prop="touzixingzhi" :formatter="investmentFormat"/>
      <el-table-column label="投资规模(元)" sortable align="center" prop="guimo"/>
      <el-table-column label="项目地点" sortable align="center" prop="siteShi" :formatter="regionFormat"/>
      <el-table-column label="预计动工时间" sortable align="center" prop="startTime">
        <template slot-scope="scope">{{ formatDateWithoutTime(scope.row.startTime) }}</template>
      </el-table-column>
      <el-table-column label="申请人" sortable align="center" prop="createUserName"/>
      <el-table-column label="流程状态" sortable align="center" prop="prostate" :formatter="processStatusFormat"/>
...
<script>
import { exportData, list,delTrackproject } from '@/api/biz/trackProject'
...
export default {
  ...
  data() {
    return {
      ...
      formPath: '/task/record/index/trackPorject',
      flowId: 'process_9q78rt6h',
      ifCardShow: false,
      showTime: '',
      flowtitle: '',
      flowopen: false,
  ...
  created() {
    ...
    this.getDicts('investment_property').then((res) => {
      this.formTouzixingzhiList = res.data
    })
    // 项目地点
  ...
  methods: {
    ...
    investmentFormat(row, column) {
      return this.selectDictLabel(this.formTouzixingzhiList, row.touzixingzhi)
    },
    // 类型翻译
    ...
    regionFormat(row, column) {
      return this.selectRegionName(this.regionAllList, row.siteShi)
    },
    handleRegionChange(value) {
      this.queryParams.siteShi = this.siteShiArr.join(',')
    },
    cardClick() {
    ...
    getList() {
      ...
      if (null != this.showTime && '' != this.showTime) {
        this.queryParams.startTimeBeg = this.showTime[0]
        this.queryParams.startTimeEnd = this.showTime[1]
      }
