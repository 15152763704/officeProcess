...
export default {
  ...
  methods: {
    ...
    // 向左旋转
    rotateLeft() {
      this.$refs.croppero.rotateLeft();
    },
    // 向右旋转
    rotateRight() {
      this.$refs.croppero.rotateRight();
    },
    // 图片缩放
    changeScale(num) {
      num = num || 1;
      this.$refs.croppero.changeScale(num);
    },
    // 上传预处理
    beforeUpload(file) {
      ...
      if(num >= 6 && this.paramsData.type=='1'){
        this.$message({
          type:"warning",
          message:"已存在六张图片，请删除后重新上传！"
        });
        this.$emit("addlist",'1')
        return false;
      }else {
    ...
    // 上传图片
    uploadImg() {
      this.$refs.croppero.getCropBlob(data => {
        let formData = new FormData();
        formData.append("file", data);
        uploadMyPhoto({type:'1'},formData).then(response => {
